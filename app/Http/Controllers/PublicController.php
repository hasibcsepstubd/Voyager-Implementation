...
//        Artisan::call("storage:link");
        ...
        $live_update = Notice::where('type', 4)->where('status', 2)->orderBy('created_at', 'desc')->first();
        $reviews = Testimonial::where('status', 2)->orderBy('created_at', 'desc')->take(10)->get();
        $faqs = Faq::where('status', 2)->get();

        return view('public.index', compact('notices', 'faqs', 'homes_info', 'rates', 'reviews', 'admin_notice','live_update'));
    }
